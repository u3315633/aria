...
const PASSWORD = "lily";     // password (EXACT match, case-sensitive)

// ===============================
// Helper to safely grab elements
// ===============================
function $(id) {
...
function playThemeTransitionAndGo() {
  // Make the transition originate from the Start button position
  if (startBtn && themeTransition) {
  ...
  // Switch body mode shortly after overlay begins
  setTimeout(() => {
    document.body.classList.remove("mode-login");
  }, 220);

  // After transition completes, go to poem
  setTimeout(() => {
    themeTransition?.classList.remove("play");
    showScreen("poem");
    initPoem();
    // reset the win fade state for restart
...
// ===============================
// Screen 2: Poem logic (Mario style typing)
// ===============================
...
function addPoemLine(text) {
  ...
  const chars = Math.max(12, Math.min(90, text.length));
  const dur = Math.max(0.9, Math.min(2.4, chars * 0.03));
  ...
  // ✅ remove cursor once typing finishes
  setTimeout(() => {
...
const herNameCard = $("herNameCard");
const issuedOn = $("issuedOn");
const serial = $("serial");

if (herNameInline) herNameInline.textContent = HER_NAME;

// Screen 4 name should be fixed value:
if (herNameCard) herNameCard.textContent = "Aria Fernandes";

// Serial should be: PASS-ID:UR2CUTE-YYYYMMDD
const now = new Date();
...
// Issued date must be exact text requested:
